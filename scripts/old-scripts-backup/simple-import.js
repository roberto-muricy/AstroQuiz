...
/**
 * Simple CSV import script - imports all questions with correct locales
 * without trying to manually link localizations
 */
...
class SimpleImporter {
  ...
  /**
   * Import all questions for all languages
   */
  async importAllQuestions(questions) {
    this.log(`\n🚀 Starting import of ${questions.length} questions in 4 languages...`);
    
    const languages = ['en', 'pt', 'es', 'fr'];
    const languageFlags = { en: '🇺🇸', pt: '🇧🇷', es: '🇪🇸', fr: '🇫🇷' };
    
    for (const language of languages) {
      this.log(`\n${languageFlags[language]} Importing ${language.toUpperCase()} questions...`);
  ...
  showSummary() {
    this.log('\n' + '='.repeat(70));
    this.log('🎉 SIMPLE IMPORT COMPLETED!');
    this.log('='.repeat(70));
    ...
    this.log(`   3. Use the locale filter to switch between languages`);
    this.log(`   4. Each locale should show the translated questions`);
    this.log(`   5. Questions with same baseId should be related through i18n`);
