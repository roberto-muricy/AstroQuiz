...
async function translateLocally() {
  ...
  const frDocIds = new Set(frQuestions.map(q => q.documentId));

  const toTranslate = enQuestions.filter(q => !frDocIds.has(q.documentId));
  console.log('✅ Already translated: ' + frQuestions.length);
