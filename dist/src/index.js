...
async function cleanupExpiredQuizSessions(strapi) {
    ...
    try {
        ...
        if (deleted > 0)
            strapi.log.info(`🧹 Deleted ${deleted} expired quiz sessions`);
    }
    catch (e) {
...
async function fetchImageUrlsByQuestionIds(strapi, questionIds) {
    ...
    const map = new Map();
    for (const r of rows || []) {
        if ((r === null || r === void 0 ? void 0 : r.questionId) && (r === null || r === void 0 ? void 0 : r.imageUrl))
            map.set(r.questionId, r.imageUrl);
    }
    return map;
}
async function fetchImageCandidateForPhase(strapi, { locale, levels, includeDrafts }) {
    ...
    let query = knex('questions as q')
        .join('files_related_mph as frm', function () {
        this.on('frm.related_id', '=', 'q.id')
            .andOnVal('frm.related_type', 'api::question.question')
            .andOnVal('frm.field', 'image');
    })
        .join('files as f', 'f.id', 'frm.file_id')
        .select('q.id', 'q.document_id as documentId', 'q.question', 'q.option_a as optionA', 'q.option_b as optionB', 'q.option_c as optionC', 'q.option_d as optionD', 'q.explanation', 'q.level', 'q.topic', 'q.locale', 'f.url as imageUrl')
...
function requireWriteTokenIfConfigured(ctx) {
    ...
    if (!requiredToken)
        return;
    const authHeader = ((_a = ctx.request.headers) === null || _a === void 0 ? void 0 : _a.authorization) || '';
...
async function fetchQuestionRowById(strapi, id) {
    ...
    const rows = await knex('questions as q')
        ...
        .select('q.id', 'q.document_id as documentId', 'q.question', 'q.option_a as optionA', 'q.option_b as optionB', 'q.option_c as optionC', 'q.option_d as optionD', 'q.correct_option as correctOption', 'q.explanation', 'q.topic', 'q.topic_key as topicKey', 'q.level', 'q.locale', 'q.base_id as baseId', 'q.question_type as questionType', 'q.published_at as publishedAt', 'q.created_at as createdAt', 'q.updated_at as updatedAt', 'f.id as imageId', 'f.url as imageUrl', 'f.name as imageName')
    ...
    async bootstrap({ strapi }) {
        ...
        strapi.server.routes([
            ...
            // Questions API routes - uses raw DB query for reliable image population
            {
                ...
                handler: async (ctx) => {
                    try {
                        const { locale = 'en', limit = 100, start = 0, baseId, questionType, published, withImage, } = ctx.query;
                        ...
                        let query = knex('questions as q')
                            ...
                            .leftJoin('files as f', 'f.id', 'frm.file_id')
                            .select('q.id', 'q.document_id as documentId', 'q.question', 'q.option_a as optionA', 'q.option_b as optionB', 'q.option_c as optionC', 'q.option_d as optionD', 'q.correct_option as correctOption', 'q.explanation', 'q.topic', 'q.topic_key as topicKey', 'q.level', 'q.locale', 'q.base_id as baseId', 'q.question_type as questionType', 'q.published_at as publishedAt', 'q.created_at as createdAt', 'f.id as imageId', 'f.url as imageUrl', 'f.name as imageName')
                            .where('q.locale', locale);
                        if (baseId)
                            query = query.andWhere('q.base_id', baseId);
                        if (published === 'true')
                            query = query.andWhereRaw("q.published_at IS NOT NULL AND q.published_at != ''");
                        if (published === 'false')
                            query = query.andWhereRaw("(q.published_at IS NULL OR q.published_at = '')");
                        if (withImage === 'true')
                            query = query.whereNotNull('f.id');
                        if (withImage === 'false')
                            query = query.whereNull('f.id');
                        if (questionType === 'image')
                            query = query.andWhereRaw("(q.question_type = 'image' OR f.id IS NOT NULL)");
                        else if (questionType === 'text')
                            query = query.andWhereRaw("(q.question_type IS NULL OR q.question_type = 'text') AND f.id IS NULL");
                        else if (questionType)
                            query = query.andWhere('q.question_type', questionType);
                        query = query.orderBy('q.id', 'desc');
                        const rows = await query;
                        // Transform to API format
                        const data = rows.slice(Number(start) || 0, (Number(start) || 0) + (Number(limit) || 100)).map((r) => ({
                            id: r.id,
                            documentId: r.documentId,
                            question: r.question,
                            optionA: r.optionA,
            ...
            },
            {
                ...
                handler: async (ctx) => {
                    ...
                    try {
                        ...
                        const data = (body === null || body === void 0 ? void 0 : body.data) && typeof body.data === 'object' ? body.data : body;
                        // Allow publishing via ?publish=true or by passing publishedAt explicitly.
                        const publish = ((_a = ctx.query) === null || _a === void 0 ? void 0 : _a.publish) === 'true';
            ...
            },
            {
                ...
                handler: async (ctx) => {
                    try {
                        const { phaseNumber = 1, locale = 'pt', excludeQuestions = [], forceImage = false, 
                        // New default behavior: ensure at least one image question if available for the phase/locale.
                        ensureImage = true, includeDrafts = false, } = ctx.request.body || {};
                        ...
                        const normalizeQuestion = (q) => {
                            var _a, _b, _c;
                            const imgUrl = (q === null || q === void 0 ? void 0 : q.imageUrl) ||
                                ((_a = q === null || q === void 0 ? void 0 : q.image) === null || _a === void 0 ? void 0 : _a.url) ||
                                (Array.isArray(q === null || q === void 0 ? void 0 : q.image) ? (_c = (_b = q.image) === null || _b === void 0 ? void 0 : _b[0]) === null || _c === void 0 ? void 0 : _c.url : null) ||
                                null;
                            // Be defensive: if we have an image URL, it's an image question (even if questionType is wrong).
                            const qType = imgUrl ? 'image' : ((q === null || q === void 0 ? void 0 : q.questionType) || (q === null || q === void 0 ? void 0 : q.question_type) || 'text');
                            return {
                                id: q.id,
                        ...
                        try {
                            const ids = (questions || []).map(q => q === null || q === void 0 ? void 0 : q.id).filter(Boolean);
                            const imageMap = await fetchImageUrlsByQuestionIds(strapi, ids);
                            questions = (questions || []).map((q) => {
                                const imgUrl = (q === null || q === void 0 ? void 0 : q.imageUrl) || imageMap.get(q === null || q === void 0 ? void 0 : q.id) || null;
                                return {
                                    ...q,
                                    imageUrl: imgUrl,
                                    questionType: imgUrl ? 'image' : ((q === null || q === void 0 ? void 0 : q.questionType) || 'text'),
                                };
                            });
                        }
                        catch (e) {
                ...
                handler: async (ctx) => {
                    try {
                        ...
                        if (questionData) {
                            ...
                            questionLevel = (fullQuestion === null || fullQuestion === void 0 ? void 0 : fullQuestion.level) || 1;
                        }
