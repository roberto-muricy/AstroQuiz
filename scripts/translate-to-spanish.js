...
async function translateLocally() {
  if (!DEEPL_API_KEY) {
    ...
    console.error('   Usage: API_TOKEN="token" DEEPL_API_KEY="key" node scripts/translate-to-spanish.js');
    process.exit(1);
  }
  ...
  // Check existing ES
  const esQuestions = await fetchAllQuestions('es');
  const esDocIds = new Set(esQuestions.map(q => q.documentId));

  const toTranslate = enQuestions.filter(q => !esDocIds.has(q.documentId));
  console.log('✅ Already translated: ' + esQuestions.length);
  ...
  // Use Pro API (key without :fx suffix)
  const DEEPL_URL = 'https://api.deepl.com/v2';
  console.log('🌍 DeepL: Pro API');
  console.log('');
  ...
  for (let i = 0; i < toTranslate.length; i++) {
    ...
    try {
      ...
      if (errors === 1) {
        ...
        console.log('DeepL error:', error.response?.data || error.message);
